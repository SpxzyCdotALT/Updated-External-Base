...
#pragma pack(pop)
			...
			const char* detail = nullptr)
		{
			if (ok)
			{
				Output::Success("Magic bullet hook installed -> orig 0x%llx, stub 0x%llx", orig, stub);
			}
			else
			{
				Output::Error("Magic bullet hook failed (%s)", detail ? detail : "unknown");
			}
		}
	}
	...
	void Ensure(bool want)
	{
		...
		{
			Remove();
			Output::Fang("Magic bullet rescan module");
		}
		...
		{
			Remove();
			Output::Info("Magic bullet removed");
		}
	}
