...
namespace ViewportSilent
{
	namespace
	{
		...
		{
			std::uintptr_t ve = visual_engine();
			if (!ve)
			{
				return {};
			}
			...
			float view[16]{};
			if (Driver->ReadRaw(ve + Offsets::VisualEngine::ViewMatrix, view, sizeof(view)) != sizeof(view))
			{
				return {};
			}

			dims_out = Driver->Read<SDK::Vector2>(ve + Offsets::VisualEngine::Dimensions);
		...
		{
			if (!Driver->IsValid(g_cam))
			{
				return;
			}

			Driver->WriteRaw(g_cam + Offsets::Camera::Viewport, &v, sizeof(v));
			g_spoofed.store(true, std::memory_order_release);
		}

		void restore_viewport()
		{
			if (!Driver->IsValid(g_cam) || g_dims.x < 1.f || g_dims.y < 1.f)
			{
				return;
			}
