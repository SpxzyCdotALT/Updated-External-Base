...
#pragma pack(pop)
		...
		{
			if (!addr_ok(a) || !d || !s || !Driver->Get_Handle())
			{
				return false;
			}
			return Driver->WriteRaw(a, d, s) == s;
		}
		...
		DWORD query_protect(std::uintptr_t a)
		{
			...
			if (!VirtualQueryEx(Driver->Get_Handle(),
		...
		{
			DWORD x = p & 0xFF;
			if (x == PAGE_EXECUTE) return true;
			if (x == PAGE_EXECUTE_READ) return true;
			if (x == PAGE_EXECUTE_READWRITE) return true;
			if (x == PAGE_EXECUTE_WRITECOPY) return true;
			return false;
		}
		...
		{
			if (!addr_ok(address) || !size || !Driver->Get_Handle())
			{
				return false;
			}
			...
			{
				DWORD old = 0;
				if (VirtualProtectEx(Driver->Get_Handle(),
				...
				ULONG  nt_old = 0;
				LONG st = nt_protect(
					Driver->Get_Handle(), &nt_base, &nt_size, req, &nt_old);
				if (st >= 0)
				{
					if (old_protect)
					{
		...
		bool region_is_padding(std::uintptr_t a, std::size_t n)
		{
			...
			if (Driver->ReadRaw(a, buf.data(), n) != n)
			{
				return false;
			}
			...
			std::uintptr_t ignore = 0)
		{
			...
			for (std::size_t mi = 0; mi < sizeof(pref) / sizeof(pref[0]); ++mi)
			{
				...
				const std::uintptr_t mod = Driver->Get_Module(name);
				if (!mod)
				{
					continue;
				}
			...
			while (VirtualQueryEx(Driver->Get_Handle(),
		...
		std::uintptr_t alloc_exec_page() {
			const std::uintptr_t p = Driver->Alloc(page_sz(), PAGE_EXECUTE_READWRITE);
			if (!p) return 0;
			...
			const char* detail = nullptr)
		{
			if (ok)
			{
				Output::Success("Raycast hook installed -> orig 0x%llx, stub 0x%llx", orig, stub);
			}
			else
			{
				Output::Error("Raycast hook failed (%s)", detail ? detail : "unknown");
			}
		}
	}
	...
	bool Install()
	{
		...
		std::uintptr_t base = Driver->Get_Module(L"RobloxPlayerBeta.exe");
		if (!base)
		{
			return false;
		}
		...
		std::uintptr_t fn = Driver->Read<std::uintptr_t>(slot);
		...
		if (!g_hook.state)
		{
			g_hook.state = Driver->Alloc(page_sz(), PAGE_READWRITE);
		}
		...
		FlushInstructionCache(Driver->Get_Handle(), (void*)stub, thunk.size());
		mark_cfg(stub);
		...
		if (!write_protected(slot, &stub, sizeof(stub)) ||
			Driver->Read<std::uintptr_t>(slot) != stub)
		{
			g_lastFail = now;
			if (owned)
			{
	...
	void Remove()
	{
		...
		if (g_hook.thunk && g_hook.thunk_owned)
		{
			Driver->Free(g_hook.thunk);
		}

		if (g_hook.state)
		{
			Driver->Free(g_hook.state);
		}
	...
	void Ensure(bool want)
	{
		...
		{
			Remove();
			Output::Fang("Raycast rescan module");
		}
		...
		if (want)
		{
			...
			{
				std::uintptr_t slot = base + desc_rva_z + bound_fn_offset;
				std::uintptr_t cur = Driver->Read<std::uintptr_t>(slot);
				if (cur != g_hook.thunk)
				{
		...
		{
			Remove();
			Output::Info("Raycast silent removed");
		}
	}
	...
	void SetActive(bool on, const SDK::Vector3& world_target, bool wallbang)
	{
		...
		{
			SDK::Instance c = Globals::Workspace.Find_First_Child_Of_Class("Camera");
			if (c.Address && Driver->IsValid(c.Address))
			{
				SDK::Camera cam_obj(c.Address);
				SDK::Vector3 cp = cam_obj.Get_CameraPos();
				cam[0] = cp.x;
				cam[1] = cp.y;
				cam[2] = cp.z;
			}
